...
let userProgress = JSON.parse(localStorage.getItem('awsHubProgress')) || { completedLessons: [], completedQuizzes: [] };
...
const curriculum = [
    {
        ...
        quiz: [
            {
                id: 'q1',
                question: 'Which IAM entity do you attach to an EC2 instance so it can securely access an S3 bucket?',
        ...
        quiz: [
            {
                ...
                options: ['Database storage', 'Virtual servers in the cloud', 'Domain Name System (DNS)', 'Content Delivery Network (CDN)'],
                correct: 1
            }
        ]
    },
...
function renderQuiz(mIndex) {
    ...
    quiz.forEach((q, i) => {
        ...
        html += \`
                ...
                <button onclick="checkAnswer('\${q.id}', \${mIndex}, \${i})" class="mt-4 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors">
                    Submit Answer
                </button>
    ...
    if (deployedServices.length === 0) {
        ...
        deployedServices.forEach(service => {
            ...
            node.innerHTML = \`
                ...
                <button class="remove-btn" onclick="removeService('\${service.id}')" title="Remove Service">
...
function generateIaC() {
    if (deployedServices.length === 0) {
        elements.iacOutput.innerHTML = '<span class="text-gray-400 italic">// Add services to generate IaC...</span>';
        return;
    }
    ...
    deployedServices.forEach(srv => {
        let resourceType = '';
        switch(srv.type) {
            case 'EC2': resourceType = 'AWS::EC2::Instance'; break;
            case 'S3': resourceType = 'AWS::S3::Bucket'; break;
            case 'RDS': resourceType = 'AWS::RDS::DBInstance'; break;
            case 'Lambda': resourceType = 'AWS::Lambda::Function'; break;
            default: resourceType = 'AWS::Custom::Resource';
        }
