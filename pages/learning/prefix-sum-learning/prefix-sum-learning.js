...
function initHeroTyping() {
  const el = document.getElementById("typingTextPrefixSum");
  if (!el) return;
...
function initExerciseToggles() {
  document.querySelectorAll(".prefix-sum-exercise-toggle").forEach((btn) => {
...
function initCopyButtons() {
  document.querySelectorAll(".prefix-sum-code-copy").forEach((btn) => {
...
function initSidebarSpy() {
  const links = document.querySelectorAll(".prefix-sum-sidebar-nav a");
  const lessons = document.querySelectorAll(".prefix-sum-lesson");
    ...
    requestAnimationFrame(() => {
      ...
      if (id) {
        ...
        const active = document.querySelector(
          `.prefix-sum-sidebar-nav a[href="#${id}"]`
        );
...
function initProgressTracker() {
  const STORAGE_KEY = "prefix-sum-learning-progress";
  const TOTAL_TOPICS = 5; // Adjust this if you change the number of topics
  const fill = document.getElementById("progressFill");
  const count = document.getElementById("progressCount");
  const bar = document.querySelector(".prefix-sum-progress-bar");
