...
function initHeroTyping() {
  const el = document.getElementById("typingTextTwoPointer");
  if (!el) return;
...
function initExerciseToggles() {
  document.querySelectorAll(".two-pointer-exercise-toggle").forEach((btn) => {
...
function initCopyButtons() {
  document.querySelectorAll(".two-pointer-code-copy").forEach((btn) => {
...
function initSidebarSpy() {
  const links = document.querySelectorAll(".two-pointer-sidebar-nav a");
  const lessons = document.querySelectorAll(".two-pointer-lesson");
    ...
    requestAnimationFrame(() => {
      ...
      if (id) {
        ...
        const active = document.querySelector(
         `.two-pointer-sidebar-nav a[href="#${id}"]`
        );
...
function initProgressTracker() {
  const STORAGE_KEY = "two-pointer-learning-progress";
  const TOTAL_TOPICS = 5; // Adjust this if you change the number of topics
  const fill = document.getElementById("progressFill");
  const count = document.getElementById("progressCount");
  const bar = document.querySelector(".two-pointer-progress-bar");
