/**
 * trees-learning.js
 * Interactivity for the Trees Learning page:
...
function initHeroTyping() {
  const el = document.getElementById("typingTextTrees");
  if (!el) return;
...
function initStatsAnimation() {
  ...
  // Fallback animation function in case global animateValue isn't loaded
  const localAnimateValue = (obj) => {
      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          if (typeof animateValue === "function") {
            ...
            localAnimateValue(entry.target);
          }
...
function initExerciseToggles() {
  document.querySelectorAll(".trees-exercise-toggle").forEach((btn) => {
...
function initCopyButtons() {
  document.querySelectorAll(".trees-code-copy").forEach((btn) => {
...
function initSidebarSpy() {
  const links = document.querySelectorAll(".trees-sidebar-nav a");
  const lessons = document.querySelectorAll(".trees-lesson");
...
function initProgressTracker() {
  const STORAGE_KEY = "trees-learning-progress";
  const TOTAL_TOPICS = 5; // Adjust this if you change the number of topics
  const fill = document.getElementById("progressFill");
  const count = document.getElementById("progressCount");
  const bar = document.querySelector(".trees-progress-bar");
