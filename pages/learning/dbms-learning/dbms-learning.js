/**
 * dbms-learning.js
 * Interactivity for the DBMS Learning page:
...
function initHeroTyping() {
  const el = document.getElementById("typingTextDbms");
  if (!el) return;
...
function initExerciseToggles() {
  document.querySelectorAll(".dbms-exercise-toggle").forEach((btn) => {
...
function initCopyButtons() {
  document.querySelectorAll(".dbms-code-copy").forEach((btn) => {
...
function initSidebarSpy() {
  const links = document.querySelectorAll(".dbms-sidebar-nav a");
  const lessons = document.querySelectorAll(".dbms-lesson");
...
function initProgressTracker() {
  const STORAGE_KEY = "dbms-learning-progress";
  const TOTAL_TOPICS = 10;
  const fill = document.getElementById("progressFill");
  const count = document.getElementById("progressCount");
  const bar = document.querySelector(".dbms-progress-bar");
